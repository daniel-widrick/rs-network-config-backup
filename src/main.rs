...
fn BackupHost(host_record : &HostRecord) -> Result<(), BackupError>{
    let backup_file_name = MakeBackupFileName(&host_record.name);
    ...
    //Run backup
    let backup_cmd = format!("/system/backup/save name={} dont-encrypt=yes",backup_file_name);
    let mut channel = sess.channel_session()?;
    channel.exec(&backup_cmd)?;

    return FetchBackup(&backup_file_name, &sess);
}
fn MakeBackupFileName(hostname : &str) -> String {
    let now:DateTime<Utc> = Utc::now();
    let date_stamp = format!("{}", now.format("%Y%m%d-%H%M"));
    return format!("{}_{}.backup",hostname,date_stamp);
}
fn FetchBackup(filename : &str, sess : &Session) -> Result<(), BackupError>{
    let (mut remote_file,stat) = sess.scp_recv(Path::new(filename))?;
